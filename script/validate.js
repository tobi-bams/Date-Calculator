...
function getdateValue(){
    ...
    var date_to2 = date_to.value;
     var date_year_to = date_to2.slice(0, 4);
     var date_month_to = date_to2.slice(5, 7);
     var date_day_to = date_to2.slice(8, 10);
     var date_year_num_to = parseInt(date_year_to);
     var date_month_num_to = parseInt(date_month_to);
     var date_day_num_to = parseInt(date_day_to);

     var date_from2 = date_from.value;
     var date_year_from = date_from2.slice(0, 4);
     var date_month_from = date_from2.slice(5, 7);
     var date_day_from = date_from2.slice(8, 10);
     var date_year_num_from = parseInt(date_year_from);
     var date_month_num_from = parseInt(date_month_from);
     var date_day_num_from = parseInt(date_day_from);

     var month_array = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]; //Array for all the days of each month in a year
  ...
  if(total_years != 0 && total_weeks != 0 && final_days != 0){
        ...
        document.getElementById('getter').innerHTML = total_years_final +", " + total_weeks_final +", "  + total_days_final;
     
  }

 if(total_years == 0){

    document.getElementById('getter').innerHTML = total_weeks_final +", "  + total_days_final;
 }
 if(total_weeks == 0){
    document.getElementById('getter').innerHTML = total_years_final +", "  + total_days_final;
 }
 if(final_days == 0){
    document.getElementById('getter').innerHTML = total_years_final +", "  + total_weeks_final;
 }

 if((total_years == 0) && (total_weeks == 0)) {
    document.getElementById('getter').innerHTML = total_days_final; 
  }
  if((total_weeks == 0) && (final_days == 0)) {
    document.getElementById('getter').innerHTML = total_years_final; 
  }
  if((total_years == 0) && (final_days == 0)) {
    document.getElementById('getter').innerHTML = total_weeks_final; 
  }

  if(date_to.value === date_from.value){
    document.getElementById('getter').innerHTML = "Same Date";
}
